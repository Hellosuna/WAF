...
query_run_main_method () {
 ...
 QUERY_SCRIPT="$QUERY_DIR/$QUERY_NAME.sql"
  ...
  if [[ -z "$DEBUG_QUERY_PART" || $DEBUG_QUERY_PART -eq 2 ]] ; then
    ...
    main_class="io.bigdatabenchmark.v2.hwc.queries.KMeansClustering"
    cmd="$BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK_SPARK_BINARY --jars ${HIVE_WH_CONNECTOR_PATH}  --class $main_class $job_jar $input --output $output --num-clusters $cluster_centers --iterations $clustering_iterations --query-num $QUERY_NAME ${initialClusters} --saveClassificationResult true --saveMetaInfo true --verbose false"
