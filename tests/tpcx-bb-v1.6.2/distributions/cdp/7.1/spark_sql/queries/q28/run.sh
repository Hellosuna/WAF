...
query_run_main_method () {
  ...
  if [[ -z "$DEBUG_QUERY_PART" || $DEBUG_QUERY_PART -eq 2 ]] ; then
    ...
    elif [[ -z "$BIG_BENCH_ENGINE_SPARK_SQL_ML_FRAMEWORK" || "$BIG_BENCH_ENGINE_SPARK_SQL_ML_FRAMEWORK" == "spark-2.4" ]]; then
          main_class="io.bigdatabenchmark.v2.queries.q28.NaiveBayesClassifier"
          job_jar="$BIG_BENCH_QUERY_RESOURCES/bigbench-ml-spark-2.4.jar"
    else
...
query_run_validate_method () {
  ...
  then
    ...
    diff -q "$VALIDATION_RESULTS_FILENAME" <(hadoop fs -cat "$RESULT_DIR/*")
    if  [ $? -eq 0 ]
    then
      echo "Validation of $VALIDATION_RESULTS_FILENAME passed: Query result matches the golden result set file"
    else
      echo "Validation of $VALIDATION_RESULTS_FILENAME may have failed: Query result doesn't match the golden result set file. Please ensure the results are within the range allowed by the benchmark specification"
      VALIDATION_PASSED="2"
    fi
    if [ "$VALIDATION_PASSED" -eq 1 ]
    then
      echo "Validation passed: Query results are OK"
    elif [ "$VALIDATION_PASSED" -eq 2 ]
    then
       echo "Validation may have passed: Check query results are within the range allowed by the benchmark specification"
    else
