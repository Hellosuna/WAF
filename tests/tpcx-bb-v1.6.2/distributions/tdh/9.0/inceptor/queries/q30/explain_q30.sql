...
CREATE TEMPORARY FUNCTION if not exists makePairs AS 'io.bigdatabenchmark.v1.queries.udf.PairwiseUDTF';
ADD FILE ${hiveconf:BIG_BENCH_HDFS_ABSOLUTE_PATH}/${hiveconf:BIG_BENCH_HDFS_QUERY_RESOURCES}/q30-sessionize.py;
...
FROM (
  ...
  REDUCE
    ...
    tstamp_inSec,
    i_category_id
  USING 'python3 q30-sessionize.py ${hiveconf:q30_session_timeout_inSec}'
  AS (
    category_id BIGINT,
    sessionid STRING
  )
