...
set hdfsDataPath=${hiveconf:BIG_BENCH_HDFS_ABSOLUTE_INIT_DATA_DIR};
set hdfsDataPath;
set fieldDelimiter=|;
set lineDelimiter='\r\n';
set tableFormat=${hiveconf:BIG_BENCH_hive_default_fileformat_source_table};
...
set clickstreamsTableBucketNum=5059;
set reviewsTableName=product_reviews;
set reviewsTableBucketNum=1171;
...
CREATE EXTERNAL TABLE ${hiveconf:customerTableName}${hiveconf:temporaryTableSuffix}
  ...
  ROW FORMAT DELIMITED FIELDS TERMINATED BY '${hiveconf:fieldDelimiter}' LINES TERMINATED BY ${hiveconf:lineDelimiter}
  STORED AS TEXTFILE LOCATION '${hiveconf:hdfsDataPath}/${hiveconf:customerTableName}'
;
-- ${hiveconf:hdfsDataPath}   /user/root/benchmarks/bigbench/data/

set ${hivevar:customerTableName};
set ${hivevar:temporaryTableSuffix};
...
set ngmr.partition.mergesize=0;
...
CREATE EXTERNAL TABLE ${hiveconf:marketPricesTableName}${hiveconf:temporaryTableSuffix}
  ...
  ROW FORMAT DELIMITED FIELDS TERMINATED BY '${hiveconf:fieldDelimiter}' LINES TERMINATED BY '\n'
  --STORED AS TEXTFILE LOCATION '${hiveconf:hdfsDataPath}/${hiveconf:marketPricesTableName}'
  STORED AS CSVFILE LOCATION '${hiveconf:hdfsDataPath}/${hiveconf:marketPricesTableName}'
;
...
set ngmr.partition.automerge=false;
