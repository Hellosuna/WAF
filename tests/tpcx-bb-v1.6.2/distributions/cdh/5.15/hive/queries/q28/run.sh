...
query_run_main_method () {
 ...
 if [ ! -r "$QUERY_SCRIPT" ]
  ...
  if [[ -z "$DEBUG_QUERY_PART" || $DEBUG_QUERY_PART -eq 2 ]] ; then
    ...
    main_class="io.bigdatabenchmark.v1.queries.q28.NaiveBayesClassifier"
    cmd="${BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK_SPARK_BINARY} --class ${main_class} ${job_jar} ${input} --output ${output} --saveClassificationResult true --saveMetaInfo true --verbose false"
