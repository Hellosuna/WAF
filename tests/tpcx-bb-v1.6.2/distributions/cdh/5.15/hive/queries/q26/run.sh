 ...
 if [ ! -r "$QUERY_SCRIPT" ]
  ...
  if [[ -z "$DEBUG_QUERY_PART" || $DEBUG_QUERY_PART -eq 2 ]] ; then
    ...
    if [[ -z "$BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK" || "$BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK" == "spark" ]]; then
      job_jar="$BIG_BENCH_QUERY_RESOURCES/bigbench-ml-spark-1x.jar"
    ...
    main_class="io.bigdatabenchmark.v1.queries.KMeansClustering"
    cmd="$BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK_SPARK_BINARY  --class $main_class $job_jar $input --output $output --num-clusters $cluster_centers --iterations $clustering_iterations --query-num $QUERY_NAME ${initialClusters} --saveClassificationResult true --saveMetaInfo true --verbose false"
...
#######
# Do not perform exact result validation for q26 since Spark's KMeans implementation produces different results based on
