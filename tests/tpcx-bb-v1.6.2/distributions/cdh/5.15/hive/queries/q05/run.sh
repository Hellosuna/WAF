...
query_run_main_method () {
 ...
 QUERY_SCRIPT="$QUERY_DIR/$QUERY_NAME.sql"
  ...
  if [[ -z "$DEBUG_QUERY_PART" || $DEBUG_QUERY_PART -eq 2 ]] ; then
    ...
    main_class="io.bigdatabenchmark.v1.queries.q05.LogisticRegression"
    cmd="$BIG_BENCH_ENGINE_HIVE_ML_FRAMEWORK_SPARK_BINARY --class ${main_class} ${job_jar} ${input} -o "${output}" $parameters --saveClassificationResult false --saveMetaInfo true --verbose false"
