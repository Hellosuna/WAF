...
query_run_main_method () {
 sed -i s/'${TEMP_RESULT_TABLE}'/$TEMP_RESULT_TABLE/g $QUERY_SQL_DIR/*.sql
 ...
 if [ ! -r "$QUERY_SCRIPT" ]
  ...
  #EXECUTION Plan:
  #step 1.  ODPS q25.sql    :  Run hive querys to extract kmeans input data
  ...
  if [[ -z "$DEBUG_QUERY_PART" || $DEBUG_QUERY_PART -eq 1 ]] ; then
    echo "========================="
    echo "$QUERY_NAME Step 1/5: Executing ODPS queries"
...
query_run_clean_method () {
  runCmdWithErrorCheck runEngineCmd --project=$BIG_BENCH_DATABASE -e "DROP TABLE IF EXISTS $TEMP_TABLE; DROP TABLE IF EXISTS $TEMP_RESULT_TABLE; DROP TABLE IF EXISTS $RESULT_TABLE;"
  return $?
}
