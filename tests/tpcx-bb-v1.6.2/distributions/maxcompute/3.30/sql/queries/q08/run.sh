...
query_run_main_method () {
	QUERY_SCRIPT="$QUERY_SQL_DIR/$QUERY_NAME.sql"
        ...
        sed -i s/'${TEMP_TABLE4}'/$TEMP_TABLE4/g $QUERY_SQL_DIR/${QUERY_NAME}.sql
...
query_run_clean_method () {
	runCmdWithErrorCheck runEngineCmd --project=$BIG_BENCH_DATABASE -e "DROP VIEW IF EXISTS $TEMP_TABLE1; DROP VIEW IF EXISTS $TEMP_TABLE2; DROP VIEW IF EXISTS $TEMP_TABLE3; DROP TABLE IF EXISTS $RESULT_TABLE;"
	return $?
}
...
query_run_validate_method () {
	...
	then
		...
		if [ `cat $RESULT_FILE | head -n 10 | wc -l` -eq 1 ]
		then
