                   ...
                   int host_shm_key)
{
    ...
    if (host_shm_key)
        LOG_VERBOSE("  host_shm_key=%d", host_shm_key);
    ...
    if (host_shm_key) {
        // map 2G-3G to host
        os_munmap(p + 2 * (uint64)BH_GB, (uint64)BH_GB);
        void *guest_shm = shmat(host_shm_key, p + 2 * (uint64)BH_GB, 0);
        if (guest_shm == (void *)-1) {
            LOG_WARNING("host share meomry failed\n");
            os_mmap(p + 2 * (uint64)BH_GB, (uint64)BH_GB, MMAP_PROT_NONE,
                    MMAP_MAP_NONE);
        }
        else if (guest_shm != (void *)(p + 2 * (uint64)BH_GB)) {
            LOG_WARNING("guest shm addr error, should be 0x%lx, but 0x%lx, go "
                        "back to mmap\n",
                        (unsigned long)(p + 2 * (uint64)BH_GB),
                        (unsigned long)guest_shm);
            shmdt(guest_shm);
            // we think it will always success.
            os_mmap(p + 2 * (uint64)BH_GB, (uint64)BH_GB, MMAP_PROT_NONE,
                    MMAP_MAP_NONE);
            os_mprotect(p + 2 * (uint64)BH_GB, (uint64)BH_GB,
                     ...
                     int host_shm_key)
{
    ...
    for (i = 0; i < memory_count; i++, memories++) {
        memory_inst = memory_instantiate(
            module_inst, module, memories, &module->memories[i], heap_size,
            error_buf, error_buf_size, i == 0 ? host_shm_key : 0);
        if (!memory_inst) {
            return false;
        }
...
aot_instantiate(AOTModule *module, bool is_sub_inst, WASMExecEnv *exec_env_main,
                ...
                uint32 error_buf_size, int host_shm_key)
{
