...
print_help()
{
    ...
    printf("  --host-shm-key=key       Host shared memory key\n");
    return 1;
}
/* clang-format on */
...
main(int argc, char *argv[])
{
        ...
        /* TODO: parse the configuration file via --addr-pool-file */
            ...
            *port_str = '\0';
            ...
            printf("iwasm %" PRIu32 ".%" PRIu32 ".%" PRIu32 "\n", major, minor,
                   patch);
            return 0;
        }else if(!strncmp(argv[0], "--host-shm-key=",15)){
            if (argv[0][15] == '\0')
                return print_help();
            host_shm_key = atoi(argv[0]+15);
        }
        else
            return print_help();
    }
    ...
    if (!(wasm_module_inst =
              wasm_runtime_instantiate(wasm_module, stack_size, heap_size,
                                       error_buf, sizeof(error_buf),host_shm_key))) {
        printf("%s\n", error_buf);
        goto fail3;
    }
